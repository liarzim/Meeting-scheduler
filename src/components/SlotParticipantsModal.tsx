...
export function SlotParticipantsModal({
  ...
  return (
    ...
    >
      <div
        className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl p-6 md:p-8 max-w-2xl w-full shadow-2xl space-y-6 max-h-[90vh] overflow-y-auto transform transition-all animate-scaleUp"
      ...
      >
        ...
        <div className="flex items-start justify-between gap-4 border-b border-slate-200 dark:border-slate-800 pb-4">
          <div className="space-y-1.5">
            <div className="flex items-center gap-2 flex-wrap">
              <span className="px-3 py-1 rounded-full bg-blue-500/10 border border-blue-500/30 text-blue-600 dark:text-blue-400 text-xs font-bold font-mono">
                ⏰ {timeRange}
              </span>
              <span
          ...
          <button
            onClick={onClose}
            className="w-9 h-9 rounded-full bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white flex items-center justify-center text-sm font-bold transition-colors"
          >
            ✕
          </button>
        </div>
        ...
        <div className="space-y-3">
          <div className="flex items-center justify-between">
            ...
            {availableParticipants.length > 0 && (
              ...
              >
                <span>📋</span>
                <span>{copiedEmails ? (language === 'he' ? 'הועתק!' : 'Copied!') : (language === 'he' ? 'העתק מיילים' : 'Copy Emails')}</span>
              </button>
            )}
          </div>
              ...
              {availableParticipants.map((p) => {
                ...
                return (
                  ...
                  >
                    ...
                    <div className="pt-2 border-t border-emerald-200/60 dark:border-emerald-800/40 space-y-1 text-xs">
                      {email && (
                        ...
                        <div className="flex items-center justify-between gap-2 pt-0.5">
                          ...
                          {cleanPhone && (
                            <a
                              ...
                              className="px-2 py-0.5 rounded-md bg-emerald-500/10 hover:bg-emerald-500/20 text-emerald-600 dark:text-emerald-400 text-[10px] font-bold transition-colors flex items-center gap-1"
                            >
                              <span>💬 WhatsApp</span>
                            </a>
                          )}
        ...
        {/* Section 2: Unavailable / Not Responded Participants (לא זמינים) */}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              {unavailableParticipants.map((p) => {
                ...
                return (
                  <div
                    key={p.id}
                    className="p-3.5 rounded-2xl bg-slate-50 dark:bg-slate-950/60 border border-slate-200 dark:border-slate-800/80 space-y-2 opacity-85 hover:opacity-100 transition-opacity text-start"
                  >
                    <div className="flex items-start justify-between gap-2">
                      <div>
                    ...
                    <div className="pt-1.5 border-t border-slate-200 dark:border-slate-800/60 flex items-center justify-between text-[11px] font-mono">
                      {email && (
                        <a
                          href={`mailto:${email}`}
